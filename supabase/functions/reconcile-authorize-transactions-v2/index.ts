...
const DEFAULT_BATCH_DAYS_BACK = 7;
const DEFAULT_MAX_BATCHES = 20;
const MAX_BATCHES = 50;
const DEFAULT_MAX_TRANSACTIONS = 25;
...
function getFallbackToken(req: Request, body?: JsonRecord) {
  const bodyAccessToken = typeof body?.accessToken === "string" ? body.accessToken.trim() : "";
  if (bodyAccessToken) return bodyAccessToken;

  const customJwt = req.headers.get("x-user-jwt") || req.headers.get("X-User-Jwt");
  if (customJwt?.trim()) return customJwt.trim();
...
async function authenticateAdmin(
  ...
  const rawAuthHeader =
    ...
    req.headers.get("authorization");
  const fallbackToken = customJwt || bodyAccessToken || null;

  const authHeader =
    (fallbackToken ? `Bearer ${fallbackToken}` : null) ||
    rawAuthHeader;

  console.log("==== AUTH DEBUG START ====");
  console.log("Authorization Header Present:", Boolean(rawAuthHeader));
  console.log("x-user-jwt Header Present:", Boolean(customJwt));
  console.log("Body accessToken Present:", Boolean(bodyAccessToken));
  console.log("Final Auth Header Present:", Boolean(authHeader));

  if (!authHeader || !authHeader.toLowerCase().startsWith("bearer ")) {
    console.log("AUTH FAIL: Missing bearer token");
    return {
      error: "Missing bearer token.",
      admin: null as AuthenticatedAdmin | null,
    };
  }
  ...
  try {
    ...
    let { data: userData, error: userError } = await authClient.auth.getUser(
      fallbackToken || undefined,
    );

    if ((userError || !userData.user) && getFallbackToken(req, body)) {
      console.log("AUTH RETRY: getUser(token)");
      const retry = await authClient.auth.getUser(getFallbackToken(req, body)!);
      userData = retry.data;
      userError = retry.error;
    }

    console.log("User Exists:", Boolean(userData.user));
    console.log("User Error:", userError?.message || null);

    if (userError || !userData.user) {
      console.log("AUTH FAIL: Invalid or expired session");
      return {
    ...
    if (!isAdmin) {
      console.log("AUTH FAIL: Not admin");
      return {
...
async function fetchPaymentTransactionsByTransactionIds(
  supabase: ReturnType<typeof createClient>,
  transactionIds: string[],
) {
  if (transactionIds.length === 0) {
    return [] as Array<{ id: string; transaction_id: string; amount: number }>;
  }

  const results: Array<{ id: string; transaction_id: string; amount: number }> = [];
  const chunks = chunkArray(transactionIds, 100);
  ...
  for (const chunk of chunks) {
    ...
    results.push(...((data || []) as Array<{ id: string; transaction_id: string; amount: number }>));
  }
...
async function syncGatewayBatches(params: {
  ...
  const limitedBatchList = batchList.slice(0, params.maxBatches);

  const summary = {
    batchesChecked: 0,
    batchesUpserted: 0,
    batchesReturned: batchList.length,
    batchesLimitedTo: params.maxBatches,
  };

  for (const batch of limitedBatchList) {
    ...
    const matchedLocalRows = await fetchPaymentTransactionsByTransactionIds(
      params.supabase,
...
async function reconcileLocalTransactions(params: {
  supabase: ReturnType<typeof createClient>;
  endpoint: string;
  apiLoginId: string;
  transactionKey: string;
  ...
  let query = params.supabase
    ...
    .limit(params.maxTransactions);

  if (params.requestedIds.length > 0) {
    query = query.in("id", params.requestedIds.slice(0, params.maxTransactions));
  }
  ...
  const summary = {
    ...
    notFound: 0,
    errored: 0,
    transactionsLimitedTo: params.maxTransactions,
  };
  ...
  return { checkedAt, summary, results };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const authResult = await authenticateAdmin(req, supabaseUrl, supabaseAnonKey, supabaseKey, body);
    if (authResult.error) {
      return jsonResponse({ success: false, error: authResult.error }, 401);
    }

    const supabase = createClient(supabaseUrl, supabaseKey);
    const mode = normalizeText(body?.mode || "sync_batches") || "sync_batches";
    const requestedIds = Array.isArray(body?.transactionIds)
      ...
      : [];
    const daysBack = Math.min(Math.max(Number(body?.daysBack || DEFAULT_BATCH_DAYS_BACK), 1), 31);
    const maxBatches = Math.min(Math.max(Number(body?.maxBatches || DEFAULT_MAX_BATCHES), 1), MAX_BATCHES);
    const maxTransactions = Math.min(
      Math.max(Number(body?.maxTransactions || body?.limit || DEFAULT_MAX_TRANSACTIONS), 1),
      MAX_TRANSACTIONS,
    );
    ...
    const reconciliation = await reconcileLocalTransactions({
      supabase,
      endpoint,
      apiLoginId,
      transactionKey,
    ...
    console.error("reconcile-authorize-transactions-v2 error:", error);
    return jsonResponse(
