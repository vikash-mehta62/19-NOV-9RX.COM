...
// Calculate balance due (order total - paid amount)
// newAmount already includes old processing fee, so this is the correct total
const orderTotalWithFee = Number(newAmount);
const balanceDueAmount = Math.max(0, Number((orderTotalWithFee - currentPaidAmount).toFixed(2)));

console.group('💰 PAYMENT ADJUSTMENT CALCULATIONS');
console.log('📊 Input Values:');
console.log('  originalAmount:', originalAmount, '(old total WITH old fee)');
console.log('  newAmount:', newAmount, '(new total WITH old fee)');
console.log('  currentPaidAmount:', currentPaidAmount);
console.log('  processingFeeAmount (old):', processingFeeAmount);
console.log('');
console.log('📈 Calculated Values:');
console.log('  differenceAmount:', differenceAmount);
console.log('  isIncrease:', isIncrease);
console.log('  orderTotalWithFee:', orderTotalWithFee);
console.log('  balanceDueAmount:', balanceDueAmount, '(amount customer owes)');
console.groupEnd();
...
// NEW CARD FEE: Only on balance due amount
...
// PROCESSOR CHARGE: Balance due + new card fee
const processorChargeAmount = Number((balanceDueAmount + cardProcessingFeeAmount).toFixed(2));

console.group('💳 CARD FEE CALCULATIONS');
console.log('  cardFeeApplies:', cardFeeApplies);
console.log('  feeSettings.cardProcessingFeePercentage:', feeSettings.cardProcessingFeePercentage);
console.log('  cardProcessingFeeAmount (NEW):', cardProcessingFeeAmount, `(${feeSettings.cardProcessingFeePercentage}% of ${balanceDueAmount})`);
console.log('  processorChargeAmount:', processorChargeAmount, '(balance + new card fee)');
console.groupEnd();
const refundCreditAmount = Math.max(
  0,
  Number((currentPaidAmount - orderTotalWithFee).toFixed(2))
    ...
    const saveOrderOnly = async () => {
      ...
      console.log('💾 SAVE ORDER ONLY - No payment adjustment');
      console.log('   newAmount (with old fee):', newAmount);
      console.log('   currentPaidAmount:', currentPaidAmount);
      ...
      try {
        ...
        if (isIncrease) {
          if (selectedAction === 'collect_payment') {
            ...
            // ✅ CORRECT BILLING MODEL - Production Safe
            // Helper for floating point precision
            const money = (n: number) => Number(n.toFixed(2));

            // Base order total (items + tax + shipping) WITHOUT any fees
            // newAmount includes OLD processing fee from ItemsTab, so subtract it
            const baseOrderTotal = money(newAmount - processingFeeAmount);
            
            // Total processing fees (old + new card fee)
            const totalProcessingFees = money(processingFeeAmount + cardProcessingFeeAmount);
            
            // Final order total INCLUDING all fees
            const newTotalAmount = money(baseOrderTotal + totalProcessingFees);
            
            // Paid amount: what was already paid + what we just charged
            const newPaidAmount = money(currentPaidAmount + processorChargeAmount);

            console.group('💾 DATABASE UPDATE - CORRECT BILLING');
            console.log('📊 Input Values:');
            console.log('  newAmount (from ItemsTab):', newAmount, '(items+tax, NO fees)');
            console.log('  processingFeeAmount (OLD):', processingFeeAmount);
            console.log('  cardProcessingFeeAmount (NEW):', cardProcessingFeeAmount);
            console.log('  currentPaidAmount:', currentPaidAmount);
            console.log('  processorChargeAmount:', processorChargeAmount);
            console.log('');
            console.log('📊 Calculated Values:');
            console.log('  baseOrderTotal:', baseOrderTotal, '(items+tax, NO fees)');
            console.log('  totalProcessingFees:', totalProcessingFees, '(old + new)');
            console.log('');
            console.log('💰 Final Database Values:');
            console.log('  total_amount:', newTotalAmount, '(base + all fees)');
            console.log('  paid_amount:', newPaidAmount, '(previous + new charge)');
            console.log('  processing_fee_amount:', totalProcessingFees);
            console.log('');
            console.log('✅ Balance Check:');
            console.log('  Total:', newTotalAmount);
            console.log('  Paid:', newPaidAmount);
            console.log('  Balance:', money(newTotalAmount - newPaidAmount));
            console.groupEnd();
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              {/* Show card processing fee breakdown if applicable */}
              {cardFeeApplies && cardProcessingFeeAmount > 0 && (
                <>
                  <div className="flex justify-between text-sm text-orange-600">
                    <span>+ New Card Processing Fee ({feeSettings.cardProcessingFeePercentage}%):</span>
                    <span className="font-medium">+${cardProcessingFeeAmount.toFixed(2)}</span>
                  </div>
                  <Separator className="my-1" />
                  <div className="flex justify-between font-bold text-lg">
                    <span className="text-blue-600">Total to Charge:</span>
            ...
            {/* Debug Card - Database Values Preview */}
            {selectedAction === 'collect_payment' && !isZeroAdjustment && (
              <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4 space-y-3">
                <div className="flex items-center gap-2 text-blue-900 font-semibold">
                  <AlertTriangle className="h-4 w-4" />
                  <span>Debug: Database Values to be Stored</span>
                </div>
                <div className="rounded-lg bg-white border border-blue-100 p-3 space-y-2 text-sm font-mono">
                  <div className="flex justify-between">
                    <span className="text-gray-600">total_amount:</span>
                    <span className="font-semibold text-blue-700">
                      ${Number((newAmount - processingFeeAmount + processingFeeAmount + cardProcessingFeeAmount).toFixed(2))}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">paid_amount:</span>
                    <span className="font-semibold text-green-700">
                      ${Number((currentPaidAmount + processorChargeAmount).toFixed(2))}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">processing_fee_amount:</span>
                    <span className="font-semibold text-orange-700">
                      ${Number((processingFeeAmount + cardProcessingFeeAmount).toFixed(2))}
                    </span>
                  </div>
                ...
                <div className="text-xs text-blue-700 space-y-1">
                  <div>• Base Amount (items+tax): ${Number((newAmount - processingFeeAmount).toFixed(2))}</div>
                  <div>• Old Processing Fee: ${processingFeeAmount.toFixed(2)}</div>
                  <div>• New Card Fee: ${cardProcessingFeeAmount.toFixed(2)}</div>
                  <div>• Current Paid: ${currentPaidAmount.toFixed(2)}</div>
                  <div>• New Charge: ${processorChargeAmount.toFixed(2)}</div>
                </div>
              </div>
            )}
          </div>
