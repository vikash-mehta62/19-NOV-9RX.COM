  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (oldOrderError || !oldOrderData) {
        console.error(
          "⚠️ Failed to fetch old order for stock reversal",
          oldOrderError
        );
      ...
      for (const item of oldOrderData.items || []) {
        if (item.sizes && item.sizes.length > 0) {
          for (const size of item.sizes) {
            ...
            if (updateError) {
              console.error(
                `❌ Failed to reverse stock for size ID ${size.id}`,
                updateError
              );
      ...
      for (const item of updatedData.items || []) {
        if (item.sizes && item.sizes.length > 0) {
          for (const size of item.sizes) {
            ...
            if (updateError) {
              console.error(
                `❌ Failed to apply new stock for size ID ${size.id}`,
                updateError
              );
      ...
      {!isEditing && (
        <>
          ...
          {userRole.toLowerCase() === "admin" &&
            form.getValues("payment.method") !== "credit" &&
            !poIs && (
              <p
                onClick={() => setModalIsOpen(true)}
                className="flex items-center gap-3 text-center justify-center px-4 py-2 text-white bg-blue-600 rounded-lg cursor-pointer hover:bg-blue-700 transition duration-300 active:scale-95 select-none"
