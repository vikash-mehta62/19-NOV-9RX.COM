...
export const ProductSizeOptions = ({
  ...
  const getSizeImageUrl = (size: any) => {
    if (size.image) {
      ...
      return `https://qiaetxkxweghuoxyhvml.supabase.co/storage/v1/object/public/product-images/${size.image}`;
    }
    // Fallback to product image
    if (product.images && product.images.length > 0) {
      const img = product.images[0];
      if (img.startsWith("http")) return img;
      return `https://qiaetxkxweghuoxyhvml.supabase.co/storage/v1/object/public/product-images/${img}`;
    }
